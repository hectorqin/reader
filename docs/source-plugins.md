...
本文区分目标架构和当前交付。local、OPDS 下载、Node 来源插件、配置界面、持久化章节出版物、富文本/图片缓存、手动刷新和服务端自动追更已经接通。外部来源通过通用进程协议接入。P1 已增加用户主动预下载与配额检查，自动缓存淘汰仍未实现。交付状态见 §13，调用示例见 [API 参考](api.md#来源与插件)。
...
| 来源类型（Source type） | 一套来源行为和配置规范，例如 `local`、`opds`、`demo-chapters`。 |
...
设计遵循 `docs/architecture.md` 的数据边界：`BOOKS_DIR` 是用户书库，是否可写由挂载权限决定；`DATA_DIR` 存数据库、下载、缓存、扫描记录和插件状态。插件不直接访问宿主数据库，也不把任意路径或远程 URL 交给客户端。
...
宿主每次来源调用预算为 60 秒，同一来源实例最多并发 2 次，宿主共 8 次，超限返回 `429 RATE_LIMITED`。插件单次 RPC 预算为 30 秒，每条 JSON 消息上限为 2 MiB（包括 base64 和协议字段）。所有 RPC（包括详情、加入书架、正文与扩展页面/操作/任务）超时或取消时，只结束当前请求：宿主发送 `$/cancelRequest`，清理该请求的计时器和监听器并忽略迟到响应，其它调用及后续请求继续使用同一进程。RPC 超时或 `TimeoutError` 信号返回 `PLUGIN_TIMEOUT`，主动取消返回 `PLUGIN_CANCELLED`。声明 `search.session` 的 `searchCancel` 控制请求不占用普通调用槽位，宿主等待确认最多 5 秒；取消或超时只结束等待，不再向其发送取消通知，让迟到的停止操作仍有机会清理搜索会话。插件应优先处理停止请求，并为断开的搜索设置空闲回收机制。

取消通知是尽力而为的协作机制，插件应按请求 ID 中断对应的网络、浏览器和解析任务；不处理通知的插件仍可能继续执行已超时的操作。超时不表示操作尚未生效，宿主不自动重复 `acquire` 等可能有副作用的调用。进程自身崩溃、输入管道损坏、协议错误或响应超过消息上限时，宿主仍会终止 worker，全部在途调用失败；进入 `failed` 后可由管理员重新启用。若插件的事件循环阻塞或内部队列被未取消任务占据，其后续调用仍可能超时，需要插件自身修复或管理员重新启用。

## 6. 内置来源

### 6.1 `local`

- 自动创建 `local` 实例，配置固定为 `{}`，使用宿主 `BOOKS_DIR`；首个切片不支持每个实例覆盖目录。文件管理写入是否可用取决于 `BOOKS_DIR` 的挂载权限。
...
当前安装流程：管理员在 Web 输入 npm 包名或上传 `.tgz`，显式确认信任代码。服务端在 `DATA_DIR/plugins` 中创建独立依赖目录，使用 npm 安装并禁用生命周期脚本，校验真实路径、manifest、入口和来源类型，保存安装记录并自动启用。同 ID 的不同版本自动进入更新流程；相同版本返回中文重复安装提示。失败清理本次目录，不影响已有插件。预部署目录加载接口仍保留，重复加载不会自动覆盖。启动时自动恢复启用的插件。启用、禁用、卸载和更新切换串行执行，禁用先注销提供者再终止进程；卸载移除安装记录，保留包文件、来源实例和已有用户数据。内置 `reader.local`、`reader.opds` 不允许卸载或替换。

已实现的在线更新流程：

1. 下载到独立安装目录，校验包、manifest、API 版本、路径和来源接口。
2. 检查已有来源实例使用的类型和能力是否仍存在，并用新版 `validateConfig` 校验每个实例的原配置（包括停用实例）。校验失败保留旧版本。
3. 若旧插件仍有在途 RPC 或管理/后台写入操作，返回 `PLUGIN_BUSY`，由管理员停止搜索或等待后重试。
4. 在同一次同步切换中更换提供者及安装记录；数据库写入或注册失败则恢复原提供者。成功后停止旧进程并启用新版。
5. 保留来源 ID、配置、凭据、订阅、插件数据目录、书架及缓存；旧包目录也保留。重启按新的安装记录恢复。

宿主不会执行插件数据迁移，也不对插件自行写入的数据提供快照回滚。插件的配置校验应无副作用，版本间需要保持自己的数据格式兼容。签名校验、自动迁移和一键版本回滚尚未实现；管理员可再次上传兼容的其它版本切换代码。
...
HTTP 返回宿主 DTO，凭据不回显。插件和来源配置管理需管理员权限；浏览、获取和个人凭据需登录。`PATCH /sources/:id` 更新名称/config/启停，config 变更清除所有用户的该来源凭据；有在途来源调用时拒绝变更 config。`DELETE` 只移除无获取记录的非本地来源。Web `#/sources` 根据类型 schema 绘制配置表单，普通成员只有浏览、凭据和个人追更操作。插件可声明 credentialKeys 与 credentials 权限，宿主只按调用用户向进程传递明确声明的键。Web 输入 npm 包名或上传安装包均支持更新同 ID 的已安装插件。
...
已交付来源管理 UI、章节富文本/插图与追更订阅；外部来源通过独立 Node.js 进程接入。后续条目不能视为已交付。
...
| 受信任 Node ProcessRunner | 已实现并测试 | JSON-RPC、30 秒调用预算、所有请求的超时/取消隔离、停止请求隔离、异常退出、2 MiB 消息上限。 |
...
| 插件安装 UI | 已实现 | 输入 npm 包名或上传 .tgz 后安装/更新并启用；保留来源配置和数据，失败保留原版本；支持启停、重启、卸载。一键版本回滚仍后续。 |
...
回归入口：在 `server` 和 `web` 分别执行 `npm test`；两端另提供 `npm run typecheck` 和 `npm run build`。测试结果以对应提交的 CI 输出为准。界面评审记录见 [书源界面](ui-review/sources-ux.md)和[书库与阅读界面](ui-review/collections-ux.md)。
...
来源实例可声明独立配置页面与后台任务，同一插件的多个实例互不共享配置。页面的 Tab、表单和输出由通用 DTO 定义，见[扩展协议](plugin-extensions.md)。

## P2 接入体验补充（2026-09-24）

来源页按 descriptor 展示能力声明，不把声明当作站点可用性保证。个人凭据页显示配置状态和最近访问结果，用户可以编辑、保留或删除指定字段；不回显秘密。

插件遇到未登录、登录过期、人工验证时应分别抛出 AUTH_REQUIRED、AUTH_EXPIRED、VERIFICATION_REQUIRED（也可作为搜索部分失败项返回）。不要将登录页面当作成功正文返回。宿主不会识别任意站点的登录页面或自动绕过验证码；实际登录、验证码和网站适配仍由插件负责。未提供会话过期时间时，界面不估算过期时间。

换源的质量检测复用 acquire / getManifest / readResource，可能拉取内嵌图片；不要在这些读取调用中加入不可重入的副作用。检测结果是一次快照，确认换源时会重新读取并验证。宿主拒绝空白正文；正文正确性、缺段与广告识别仍须插件验证。

生态协议和客户端覆盖范围见 [独立方案](ecosystem-roadmap.zh-CN.md)，交付证据见 [P2 记录](p2-progress-2026-09-24.md)。
