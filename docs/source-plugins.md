...
本文区分目标架构和当前交付。local、OPDS 下载、Node 来源插件、配置界面、持久化章节出版物、富文本/图片缓存、手动刷新和服务端自动追更已经接通。外部来源通过通用进程协议接入。设备只缓存已读章节，整书预下载和缓存淘汰仍未实现。交付状态见 §13，调用示例见 [API 参考](api.md#来源与插件)。
