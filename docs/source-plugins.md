...
宿主每次来源调用预算为 60 秒，同一来源实例最多并发 2 次，宿主共 8 次，超限返回 `429 RATE_LIMITED`。插件单次 RPC 预算为 30 秒，每条 JSON 消息上限为 2 MiB（包括 base64 和协议字段）。声明 `search.cancel` 的搜索请求支持合作式取消：宿主发送 `$/cancelRequest`，清理该请求的计时器并忽略迟到响应，其它调用继续运行。声明 `search.session` 的 `searchCancel` 控制请求不占用普通调用槽位，宿主等待确认最多 5 秒；取消或超时只结束等待，不终止插件，让迟到的停止操作仍有机会清理搜索会话。插件应优先处理停止请求，并为断开的搜索设置空闲回收机制。

其它不支持合作式取消的请求被取消、普通 RPC 超时、协议错误或进程异常退出时，宿主会终止 worker，全部在途调用失败。插件进入 `failed` 后可通过管理员重新启用来加载新进程。重试由调用方显式决定，不自动重复 `acquire` 等可能有副作用的调用。
...
| 受信任 Node ProcessRunner | 已实现并测试 | JSON-RPC、30 秒调用预算、搜索合作式取消、停止请求隔离、其它请求硬取消、异常退出、2 MiB 消息上限。 |
