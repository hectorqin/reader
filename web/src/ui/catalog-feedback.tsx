...
export function CatalogFeedback({ page, merged = false, searching = false, count = page.items.length, compact = false }: { page: SourcePage; merged?: boolean; searching?: boolean; count?: number; compact?: boolean }) {
  return <div className="catalog-feedback">
    {!compact && <div className="catalog-summary" role="status"><strong>{merged ? '已找到' : '本页'} {count} 本书</strong>
    ...
    {!!page.errors?.length && <details className="catalog-errors" open={!compact && !page.items.length && !searching}>
