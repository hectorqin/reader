...
describe('sources and subscriptions UI', () => {
  ...
  it('requires the explicit trust control before installing and offers plugin lifecycle actions', async () => {
    ...
    input(screen.element, '已部署的插件目录名', 'external-source');
    ...
    expect(JSON.parse(bodyText(transport.requests.find((request) => request.url.endsWith('/plugins') && request.method === 'POST')))).toEqual({ folder: 'external-source', trusted: true });
