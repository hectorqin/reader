...
用系统自带的语音合成，不需要联网、不额外占容器的体积，也不把你的书上传到任何服务。
...
- **语音可选**：列出系统里所有可用语音（含语言标签），默认「跟随系统」。
- **章节播完**：可选「继续下一章」或「停在章末」——后者就是有声书的「一次一章」。
- **高亮**：正在朗读的句子用半透明底色标出，并用 `Range` 绘制，**不改写书的 DOM**，
  所以开始朗读不会让章节重新分页。

> 浏览器端朗读依赖 `speechSynthesis`（Chrome / Edge / Android WebView 都有）。
> 没有该 API 的环境里，设置面板会直接说明，而不是给一个按了没反应的按钮。
...
cd web
npm install
npm test            # 173 个测试用例（vitest 147 + node:test 26，两个运行器各管一半）
...
- **漫画目录的嵌套有深度上限**：卷目录下面还有目录时会继续往下收页，但到第 4 层为止 ——
  再深会把一个杂乱目录变成几千页的巨型漫画。页序是真自然序（`1, 2, 10`，不是 `1, 10, 2`），
  排错不是观感问题，是故事被静默打乱。
