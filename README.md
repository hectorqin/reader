...
- **空状态给一条出路**：空书库不是「这里什么都没有」，而是「打开书库管理 / 刷新」两个按钮；
...
- **排序**：最近更新 / 最近入库 / 书名 / 作者，书架上一键切换，并记住选择。
...
- **书库管理**：书架右上角的 🗂 打开文件管理器，看的是 `BOOKS_DIR` 本身而不是目录库。
- **离线优先**：先画本地镜像再换成服务端数据，离线时书架照常可用；
  离线下的排序规则与服务端一致，所以断网不会让书架重排。

### 书库管理

书架回答「我能读什么」，书库管理回答「磁盘上有什么」。两件事都需要，因为书籍记录里
**从不带路径**——「刚扫完目录，可为什么书架上没有这本书」这个问题，在文件管理器之外
没有任何地方能回答。
...
**书库管理**（读写 `/books`；挂载只读时任何写操作返回 `403 READ_ONLY_MOUNT`）
